...
contract TicketMarketplace is ITicketMarketplace {
    ...
    function createEvent(uint128 maxTickets, uint256 pricePerTicket, uint256 pricePerTicketERC20) external onlyOwner override {
    ...
    function setMaxTicketsForEvent(uint128 eventId, uint128 newMaxTickets) external onlyOwner override {
    ...
    function setPriceForTicketETH(uint128 eventId, uint256 price) external onlyOwner override {
        events[eventId].pricePerTicket = price;
        emit PriceUpdate(eventId, price, "ETH");
    }

    function setPriceForTicketERC20(uint128 eventId, uint256 price) external onlyOwner override {
